...
const string strMessageMagic = "pitbull Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // pitbull: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // pitbull: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// miner's coin stake reward based on coin age spent (coin-days) FIXED PoS SUBSIDY REWARD [10512000 Approx. BULL minted every year]
...
// pitbull: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // pitbull: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // pitbull: clean up wallet after disconnecting coinstake
...
if(IsProofOfWork())
    ...
    if (IsProofOfStake())
    {
        // pitbull: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // pitbull: track money supply and mint amount info
...
// pitbull: total coin age spent in transaction, in the unit of coin-days.
...
// pitbull: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // pitbull: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // pitbull: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // pitbull: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // pitbull: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // pitbull: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // pitbull: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // pitbull: getblocks may not obtain the ancestor block rejected
    ...
    // pitbull: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "pitbull", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "pitbull born a new era";
        CTransaction txNew;
        txNew.nTime = 1613325695;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1613325695;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1068552;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xa101b4b8eeaa0680a63d5d200ed8879a317b6fe13c6dff10701e654ac713f5a6"));
        block.print();
        ...
        // pitbull: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // pitbull: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // pitbull: tell downloading node about the latest block if it's
